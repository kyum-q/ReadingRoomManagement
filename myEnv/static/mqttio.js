...
function startConnect() { // 접속을 시도하는 함수
    ...
    // client 객체에 콜백 함수 등록
    client.onConnectionLost = onConnectionLost; // 접속이 끊어졌을 때 실행되>는 함수 등록
    client.onMessageArrived = onMessageArrived; // 메시지가 도착하였을 때 실>행되는 함수 등록

    // 브로커에 접속. 매개변수는 객체 {onSuccess : onConnect}로서, 객체의 프>로퍼틴느 onSuccess이고 그 값이 onConnect.
...
function onConnectionLost(responseObject) { // 매개변수인 responseObject는 응답 패킷의 정보를 담은 개체
    document.getElementById("messages").innerHTML += '<span>오류 : 접속 끊어>짐</span><br/>';
