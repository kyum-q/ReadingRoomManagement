...
function subscribe(topic) { // subscribe
...
function publish(topic, msg) { // publish
...
// 접속이 끊어졌을 때 호출되는 함수
function onConnectionLost(responseObject) { // 매개변수인 responseObject는 응답 패킷의 정보를 담은 객체
...
function onMessageArrived(msg) { // 매개변수 msg는 도착한 MQTT 메시지를 담고 있는 객체
    ...
    if (msg.destinationName == 'ultrasonic') { // 초음파센서 토픽
...
function warning() { // 경고 주기
    document.getElementById("warning").innerHTML += '<span>!!!호출!!!</span><br/>';
}
